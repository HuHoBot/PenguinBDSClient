...
- **🔌 附属插件**：已加载的附属插件列表（名称 / 版本 / 命令数 / 描述 / 作者）
- **📦 插件中心**：浏览/搜索 [HuHoBot 附属插件中心](https://addon.txssb.cn)，一键下载安装到 `addons/` 并热加载
- **⚙️ 配置**：分组表单编辑全部配置，**保存后自动热重载生效**
- 移动端自动折叠为汉堡菜单

控制台同样支持：`huhobot center [搜索词]` / `huhobot install <插件ID> [force]`。
...
!!! warning "需要先完成 QQ 凭据配置"
    ...
    但**收发消息、群指令、指令面板等一切 QQ 功能都不可用**。请先完成[快速开始](https://beeeee-really.github.io/huhobot-docs/)的凭据配置。
