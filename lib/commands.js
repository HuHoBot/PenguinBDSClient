...
function buildOnlineMarkdown(bot, players) {
    ...
    if (template !== null && template.indexOf('{{.tps}}') === -1) {
    ...
    if (stats) {
        lines.push('TPS：' + tickIcon(stats) + ' **' + stats.tps + '**');
    }
    if (players.length) {
        lines.push('');
        ...
        execute(ctx) {
            ...
            for (const a of addons) {
                lines.push('- ' + a.name + (a.version ? ' v' + a.version : '') +
                    (a.commandCount ? '，' + a.commandCount + ' 条命令' : '') +
...
function handleGroupMessage(bot, message) {
    ...
    // 外部注册的兜底处理器（如 Llama 版的 AI Agent）
    if (_fallbackHandler) {
        try {
            if (_fallbackHandler(bot, message)) return true;
        } catch (e) {
            log.error('[HuHoBotPenguin] 兜底处理器异常：' + (e && e.message || e));
        }
    }
    return false;
}

// ---- 兜底处理器注册（供 Llama 版 Agent 注册） ----

let _fallbackHandler = null;

/**
 * 注册兜底消息处理器：所有内置命令 + 全量转发均未命中时调用。
 * @param {function} fn fn(bot, message) → boolean
 */
function registerFallbackHandler(fn) {
    _fallbackHandler = fn;
}

module.exports = { handleGroupMessage, dispatch, normalizeContent, COMMAND_NAMES, COMMANDS, registerFallbackHandler };