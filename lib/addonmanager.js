...
class AddonContext {
    constructor(manager, meta) {
        ...
        this._joinIds = [];
        this._memberJoinIds = [];
        this._memberLeaveIds = [];
        this._botJoinIds = [];
        this._botLeaveIds = [];
        this._groupNotifyIds = [];
        this._interactionIds = [];
        this._readyIds = [];
        this._cmdKeys = [];
        this._regexIds = [];
        this._unloadFns = [];
    ...
    onMemberJoin(fn) { const id = this.manager.adapter.onMemberJoin(fn); this._memberJoinIds.push(id); return id; }
    offMemberJoin(id) { this.manager.adapter.offMemberJoin(id); this._memberJoinIds = this._memberJoinIds.filter((x) => x !== id); }
    onMemberLeave(fn) { const id = this.manager.adapter.onMemberLeave(fn); this._memberLeaveIds.push(id); return id; }
    offMemberLeave(id) { this.manager.adapter.offMemberLeave(id); this._memberLeaveIds = this._memberLeaveIds.filter((x) => x !== id); }
    onBotJoinGroup(fn) { const id = this.manager.adapter.onBotJoinGroup(fn); this._botJoinIds.push(id); return id; }
    offBotJoinGroup(id) { this.manager.adapter.offBotJoinGroup(id); this._botJoinIds = this._botJoinIds.filter((x) => x !== id); }
    onBotLeaveGroup(fn) { const id = this.manager.adapter.onBotLeaveGroup(fn); this._botLeaveIds.push(id); return id; }
    offBotLeaveGroup(id) { this.manager.adapter.offBotLeaveGroup(id); this._botLeaveIds = this._botLeaveIds.filter((x) => x !== id); }
    onGroupNotifySwitch(fn) { const id = this.manager.adapter.onGroupNotifySwitch(fn); this._groupNotifyIds.push(id); return id; }
    offGroupNotifySwitch(id) { this.manager.adapter.offGroupNotifySwitch(id); this._groupNotifyIds = this._groupNotifyIds.filter((x) => x !== id); }
    onInteraction(fn) { const id = this.manager.adapter.onInteraction(fn); this._interactionIds.push(id); return id; }
    offInteraction(id) { this.manager.adapter.offInteraction(id); this._interactionIds = this._interactionIds.filter((x) => x !== id); }
    ...
    /**
     * 注册运行时命令。自动关联到本附属插件（等价于 5 参版 registerBotCommand）。
     * 也可显式传 addonName 覆盖：registerBotCommand(addonName, key, command, permission, pushMenu)。
     */
    registerBotCommand() {
        const args = Array.prototype.slice.call(arguments);
        let ok;
        if (args.length >= 5) {
            ok = this.manager.adapter.registerBotCommand.apply(this.manager.adapter, args);
            if (ok) this._cmdKeys.push(String(args[1] || '').trim());
        } else {
            ok = this.manager.adapter.registerBotCommand(this.name, args[0], args[1], args[2], args[3]);
            if (ok) this._cmdKeys.push(String(args[0] || '').trim());
        }
        return ok;
    }
    ...
    sendGroupMarkdown(groupOpenId, markdown, msgId, opts) { return this.manager.adapter.sendGroupMarkdown(groupOpenId, markdown, msgId, opts); }
    sendGroupKeyboard(groupOpenId, content, keyboard, msgId, opts) { return this.manager.adapter.sendGroupKeyboard(groupOpenId, content, keyboard, msgId, opts); }
    ...
    recallGroupMessage(groupOpenId, messageId) { return this.manager.adapter.recallGroupMessage(groupOpenId, messageId); }
    recallPrivateMessage(userOpenId, messageId) { return this.manager.adapter.recallPrivateMessage(userOpenId, messageId); }
    ackInteraction(interactionId, code) { return this.manager.adapter.ackInteraction(interactionId, code); }
    ...
    _unload() {
        ...
        for (const id of this._memberJoinIds) a.offMemberJoin(id);
        for (const id of this._memberLeaveIds) a.offMemberLeave(id);
        for (const id of this._botJoinIds) a.offBotJoinGroup(id);
        for (const id of this._botLeaveIds) a.offBotLeaveGroup(id);
        for (const id of this._groupNotifyIds) a.offGroupNotifySwitch(id);
        for (const id of this._interactionIds) a.offInteraction(id);
        ...
        this._memberJoinIds = []; this._memberLeaveIds = [];
        this._botJoinIds = []; this._botLeaveIds = [];
        this._groupNotifyIds = []; this._interactionIds = [];
...
class AddonManager {
    ...
    /**
     * 卸载单个附属插件（仅撤销运行时，不删文件）。
     * @param {string} name 已加载名称或目录名
     */
    unload(name) {
        ...
        if (!this.loaded.has(key)) {
            for (const [n, ctx] of this.loaded.entries()) {
                const folder = path.basename(ctx.manager.dir);
                // meta 中无 folder，用加载时 entryName——存一下更好；用 loaded key 对照
                if (n === key) { entryName = n; break; }
            }
        }
        const ctx = this.loaded.get(entryName) || this.loaded.get(key);
        if (!ctx) return false;
        ctx._unload();
        this.loaded.delete(entryName);
        log.info('[HuHoBotPenguin] 附属插件已卸载：' + entryName);
        return true;
    }

    /**
     * 卸载并删除 addons 下的插件目录/文件。
     * @param {string} name 目录名或已加载名
     * @returns {{ok:boolean, folder?:string, error?:string}}
     */
    uninstall(name) {
        const key = String(name || '').trim();
        if (!key) return { ok: false, error: '名称为空' };
        // 先按已加载名卸载
        let folder = key;
        if (this.loaded.has(key)) {
            folder = key;
            ...
            // 尝试用 meta 名卸载
            for (const [n] of Array.from(this.loaded.entries())) {
        ...
        const dirPath = path.join(this.dir, folder);
        const jsPath = path.join(this.dir, folder + '.js');
        try {
            if (fs.existsSync(dirPath) && fs.statSync(dirPath).isDirectory()) {
                ...
                return { ok: false, error: '未找到插件目录：' + folder };
            }
            log.info('[HuHoBotPenguin] 附属插件已删除：addons/' + folder);
    ...
    listInstalledMeta() {
        ...
        for (const ent of entries) {
            if (ent.isDirectory()) {
                let meta = { folder: ent.name, name: ent.name, version: '', loaded: this.loaded.has(ent.name) };
                const mp = path.join(this.dir, ent.name, 'addon.json');
                if (fs.existsSync(mp)) {
                    try {
                        const j = JSON.parse(fs.readFileSync(mp, 'utf8'));
                        meta.name = String(j.name || ent.name).trim();
                        meta.version = String(j.version || '').trim();
                        meta.description = String(j.description || '').trim();
                        meta.author = String(j.author || '').trim();
                    } catch (e) { /* ignore */ }
                }
                // 已加载名可能与 folder 不同
                if (this.loaded.has(meta.name)) meta.loaded = true;
                out.push(meta);
            } else if (ent.isFile() && ent.name.endsWith('.js')) {
                const folder = ent.name.slice(0, -3);
                out.push({ folder, name: folder, version: '', loaded: this.loaded.has(folder), description: '', author: '' });
            }
        }
        return out;
    }
