...
/**
 * 实时 TPS 监测（BDS 无 tps 命令，自行测量）。
 * mc.listen("onTick") 每个 tick 触发一次，滚动窗口（最近 100 tick）
 * 内根据 tick 间隔推算 TPS。
 */
...
class TickMonitor {
    ...
    start() {
        ...
        try {
            ...
            log.warn('[HuHoBotPenguin] onTick 监听注册失败，TPS 统计不可用：' + (e && e.message || e));
            return false;
        }
    }
    ...
    /**
     * 最近窗口统计。
     * @returns {{tps: number, ticks: number} | null} 数据不足返回 null
     */
    getStats() {
